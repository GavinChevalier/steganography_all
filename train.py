...
@click.command()
# @click.option('--train_dir', default=r'F:\Gavin\data\LSUN\living_room', help='Data path for training')
# @click.option('--validation_dir', default=r'F:\Gavin\data\LSUN\living_room_val', help='Data path for valication')

# @click.option('--train_dir', default=r'F:\Gavin\data\celeba', help='Data path for training')
# @click.option('--validation_dir', default=r'F:\Gavin\data\celeba_test', help='Data path for valication')

@click.option('--train_dir', default=r'/data/gavin/Gavin/data/celeba', help='Data path for training')
@click.option('--validation_dir', default=r'/data/gavin/Gavin/data/celeba_test', help='Data path for valication')
...
# @click.option('--output_path', default=r'F:\Gavin\0000\WGAN\wganoutput_nz%d_livingroom'%nz, help='Output path where result (.e.g drawing images, cost, chart) will be stored')
# @click.option('--output_path', default=r'F:\Gavin\0000\WGAN\wganoutput_nz%d_celeba_correctground'%nz, help='Output path where result (.e.g drawing images, cost, chart) will be stored')
@click.option('--output_path', default=r'/data/gavin/extra/WGAN/wganoutput_nz%d_celeba'%nz, help='Output path where result (.e.g drawing images, cost, chart) will be stored')
...
@click.option('--saving_step', default=100, help='Save model, sample every this saving step')
...
def train(train_dir, validation_dir, image_data_type, output_path, dim, lr, critic_iters, gen_iters, batch_size, noisy_label_prob, start_iter, end_iter, gp_lambda, num_workers, saving_step, training_class, val_class, restore_mode):
    ...
    for iteration in range(start_iter, end_iter):
        ...
        for i in range(critic_iters):
            ...
            # samples_ground = real_data.view(batch_size, 3, dim, dim)
            # samples_ground = samples_ground * 0.5 + 0.5
            # torchvision.utils.save_image(samples_ground, str(sample_path / 'samples_ground_{}.png').format(iteration),
            #                              nrow=8,
            #                              padding=2)
